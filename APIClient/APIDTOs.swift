...
/// DTO для позиции портфеля (результат маппинга из API)
...
// MARK: - Cash Operations API DTO

/// DTO для денежной операции из API
struct CashOperationDTO: Decodable {
    let date: String
    let type: String
    let amount: Double
    let currency: String
